...
- **Camera:** Closer **room-locked** **straight overhead (top-down)** view (CELL=112 ≈14×10 tiles). **Fits/centers the whole room** when it fits; otherwise **follow-camera** with mid-wall clamp (neighboring rooms stay off-screen)
...
- Closer camera (canvas 1600×1120, CELL=112 → ~14×10 tiles); rooms often need soft follow
...
- View: **straight overhead (top-down)** — axis-aligned floor tiles; light faux-3D wall thickness only (still reads overhead)
- Floors: **sand + irregular cobblestone**
- Walls: **weathered stone brick** tops with a light south face for thickness; doorways read clearly
- Sprites drawn on the floor plane; warm torch glow; underground abandoned-tunnel vibe
- Characters & pickups drawn larger (entity sprite scale `SP ≈ PX*2`); closer CELL=112 zoom
- Creatures only aggro with **clear line of sight** (short memory fade); without LOS they **aimless-wander**. Behind-grab only after spotting and closing in. Flashlight still scares minions
...
- `js/game.js` — gameplay, closer room-locked top-down camera, LOS-only AI, sprint, keys/gate, enemies, QTE, HUD, overhead render
